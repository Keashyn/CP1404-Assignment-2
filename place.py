    ...
    def mark_places(self,xy):
        ...
        if self.sorted_data[x][3] != "v":
            ...
            if self.priority_place(x):
                ...
                self.root.ids.status.text="You visited {0} in {1}. Great travelling!".format(self.sorted_data[x][0], self.sorted_data[x][1])
                """if its important or True it will print this"""
            else:
                self.root.ids.status.text="You visited {0} in {1}.".format(self.sorted_data[x][0], self.sorted_data[x][1])
            ...
            if self.sorted_data(x):
                """This checks if the place is important or not and if its True it will give a different print out in the status bar"""
                self.root.ids.status.text="You need to visit {0} in {1}. Get going!".format(self.sorted_data[x][0],self.sorted_data[x][1])
                """if its important or True it will print this"""
            else:
                self.root.ids.status.text="You need to visit {0} in {1}.".format(self.sorted_data[x][0],self.sorted_data[x][1])
                """if its important or False it will print this"""
